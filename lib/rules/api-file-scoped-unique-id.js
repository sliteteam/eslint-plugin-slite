module.exports = {
  meta: {
    type: "problem",
    messages: {
      oneObjectArgument: `fileScopedUniqueId argument must be only one Object`,
      noFilename: `fileScopedUniqueId object argument must contain "filename: __filename"`,
      noId: `fileScopedUniqueId object argument must contain a key "id"`,
      nonLiteralId: `fileScopedUniqueId argument must be contain an "id" as a string literal ("example-id" or 'example-id') to be statically analyzed.`,
      nonUniqueId: `fileScopedUniqueId "id" : "{{nonUniqueValue}}" already exists this file.`,
    },
  },
  create(context) {
    ...
    return {
      CallExpression(node) {
        if (node.callee.type !== "Identifier" || node.callee.name !== "fileScopedUniqueId") {
          return;
        }
