...
function findContextTypeFromFileName(filename) {
  ...
  switch (type) {
    ...
    case "worker":
      return { pascalCase: "Worker", contextType: "WORKER" };
    default:
      return {};
  }
}
    ...
    return {
      ImportDeclaration(node) {
        if (serviceAlreadyImported === false) {
          serviceAlreadyImported = Boolean(
            context
      ...
      onCodePathEnd(codePath, node) {
        ...
        if (
          node.type === "Program" &&
          isContext(filepath) &&
          foundCreateSliteContextUsage === false
        ) {
          const loc = {
            ...
            suggest: [
              {
                ...
                fix: function (fixer) {
                  ...
                  const { pascalCase, contextType } =
                    findContextTypeFromFileName(filename);

                  const name = capitalizeFirstLetter(
                    camelize(filename.split("-").slice(1).join("-"))
                  );
...
`;
                  const filepathAfterContext = filepath
                    .split("/contexts/")
                    .pop();
                  const pathToSrc = filepathAfterContext
                    .split("/")
                    .map(() => "..")
                    .join("/");
                  const importCode = [
                    `import { createSliteContext, SliteContextType } from '${pathToSrc}/helpers/contexts'`,
                  ];
                  if (!serviceAlreadyImported) {
                    importCode.push(
                      `import { Services } from '${pathToSrc}/services'`
                    );
                  }
                  importCode.push("");

                  return [
                    fixer.insertTextBefore(node, importCode.join("\n")),
